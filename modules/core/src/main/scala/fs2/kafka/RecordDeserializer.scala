...
sealed abstract class RecordDeserializer[F[_], A] {
  ...
  /** Returns a new [[RecordDeserializer]] instance that will catch deserialization errors and
    * return them as a value, allowing user code to handle them without causing the consumer to
    * fail.
    */
