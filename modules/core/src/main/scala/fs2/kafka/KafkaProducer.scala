...
/** [[KafkaProducer]] represents a producer of Kafka records, with the ability to produce
  * `ProducerRecord`s using [[produce]]. Records are wrapped in [[ProducerRecords]] which allow an
  * arbitrary value, that is a passthrough, to be included in the result. Most often this is used
  * for keeping the [[CommittableOffset]]s, in order to commit offsets, but any value can be used as
  * passthrough value.
  */
...
object KafkaProducer {
  ...
  implicit class ProducerOps[F[_], K, V](private val producer: KafkaProducer[F, K, V])
    ...
    /** Produce a single [[ProducerRecord]] without a passthrough value, see
    ...
    /** Produce a single record to the specified topic using the provided key and value without a
      * passthrough value, see [[KafkaProducer.produce]] for general semantics.
      */
    ...
    /** Produces the specified [[ProducerRecords]] without a passthrough value, see
      * [[KafkaProducer.produce]] for general semantics.
      */
    def produce_(
  ...
  /** Creates a [[KafkaProducer]] using the provided settings and produces record in batches,
    * limiting the number of records in the same batch using [[ProducerSettings#parallelism]].
    */
  def pipe[F[_], K, V, P](
      settings: ProducerSettings[F, K, V]
  )(implicit
  ...
  /** Produces records in batches using the provided [[KafkaProducer]]. The number of records in the
    * same batch is limited using the [[ProducerSettings#parallelism]] setting.
    */
