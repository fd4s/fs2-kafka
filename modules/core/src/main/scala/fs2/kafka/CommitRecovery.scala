...
/** [[CommitRecovery]] describes how to recover from exceptions raised while trying to commit
  ...
  * recover from any exceptions, you can use [[CommitRecovery#None]].<br> <br> To create a new
  * [[CommitRecovery]], simply create a new instance and implement the [[recoverCommitWith]]
  * function with the wanted recovery strategy. To use the [[CommitRecovery]], you can simply set it
  * with [[ConsumerSettings#withCommitRecovery]].
  */
abstract class CommitRecovery {
  ...
  /** Describes recovery from offset commit exceptions. The `commit` parameter can be used to retry
    ...
    * be handled and recovered.<br> <br> The offsets we are trying to commit are available via the
    * `offsets` parameter. Waiting before retrying again can be done via the provided `Timer`
    * instance, and jitter can be applied using the `Jitter` instance.
    */
...
object CommitRecovery {
  ...
  /** The default [[CommitRecovery]] used in [[ConsumerSettings]] unless a different one has been
    ...
    * raise a [[CommitRecoveryException]] with the last such error experienced.<br> <br> The sum of
    * time spent waiting between retries will always be less than 70 220 milliseconds, or ~70
    * seconds. Note that this does not include the time for attempting to commit offsets. Offset
