...
/** [[ProducerSettings]] contain settings necessary to create a [[KafkaProducer]]. At the very
  * least, this includes a key serializer and a value serializer.<br> <br> Several convenience
  * functions are provided so that you don't have to work with `String` values and `ProducerConfig`
  * for configuration. It's still possible to specify `ProducerConfig` values with functions like
  * [[withProperty]].<br> <br> [[ProducerSettings]] instances are immutable and all modification
  * functions return a new [[ProducerSettings]] instance.<br> <br> Use `ProducerSettings#apply` to
...
sealed abstract class ProducerSettings[F[_], K, V] {
  ...
  /** The time to wait for the Java `KafkaProducer` to shutdown.<br> <br> The default value is 60
    * seconds.
    */
  ...
  /** The maximum number of [[ProducerRecords]] to produce in the same batch.<br> <br> The default
    * value is 10000.
    */
  def parallelism: Int
