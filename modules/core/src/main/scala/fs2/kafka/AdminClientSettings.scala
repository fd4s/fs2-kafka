...
/** [[AdminClientSettings]] contain settings necessary to create a [[KafkaAdminClient]]. Several
  ...
  * like [[withProperty]].<br> <br> [[AdminClientSettings]] instances are immutable and all
  * modification functions return a new [[AdminClientSettings]] instance.<br> <br> Use
  * [[AdminClientSettings#apply]] for the default settings, and then apply any desired modifications
...
sealed abstract class AdminClientSettings {
  ...
  /** The time to wait for the Java `KafkaAdminClient` to shutdown.<br> <br> The default value is 20
    * seconds.
    */
