...
/** [[ProducerRecords]] represents zero or more `ProducerRecord`s, together with an arbitrary
  * passthrough value, all of which can be used with [[KafkaProducer]]. [[ProducerRecords]]s can be
  * created using one of the following options.<br> <br>
  *   - `ProducerRecords#apply` to produce zero or more records and then emit a [[ProducerResult]]
  *     with the results and specified passthrough value.<br>
  *   - `ProducerRecords#one` to produce exactly one record and then emit a [[ProducerResult]] with
  *     the result and specified passthrough value.<br> <br> The [[passthrough]] and [[records]] can
  *     be retrieved from an existing [[ProducerRecords]] instance.<br>
  */
...
object ProducerRecords {
  ...
  /** Creates a new [[ProducerRecords]] for producing zero or more `ProducerRecords`s, then emitting
    * a [[ProducerResult]] with the results and `Unit` passthrough value.
    */
  def apply[F[+_], K, V](
  ...
  /** Creates a new [[ProducerRecords]] for producing zero or more `ProducerRecords`s, then emitting
    * a [[ProducerResult]] with the results and specified passthrough value.
    */
  def apply[F[+_], P, K, V](
      records: F[ProducerRecord[K, V]],
      passthrough: P
    ...
    val chunk = if (numRecords <= 1) {
      ...
      F.foldLeft(records, ()) { case (_, record) =>
        buf += record
        ()
      }
  ...
  /** Creates a new [[ProducerRecords]] for producing exactly one `ProducerRecord`, then emitting a
    * [[ProducerResult]] with the result and `Unit` passthrough value.
    */
  def one[K, V](
  ...
  /** Creates a new [[ProducerRecords]] for producing exactly one `ProducerRecord`, then emitting a
    * [[ProducerResult]] with the result and specified passthrough value.
    */
  def one[P, K, V](
      record: ProducerRecord[K, V],
      passthrough: P
