...
object Serializer {
  ...
  /** Creates a new [[Serializer]] which delegates serialization to the specified Kafka
    * `Serializer`. Note the `close` and `configure` functions won't be called for the delegate.<br>
    * <br> It is assumed the delegate `serialize` function is pure. If it's not pure, then use
