...
object Deserializer {
  ...
  /** Creates a new [[Deserializer]] which delegates deserialization to the specified Kafka
    ...
    * delegate.<br> <br> It is assumed the delegate `deserialize` function is pure. If it's not
    * pure, then use `suspend` after `delegate`, so the impure behaviours can be captured properly.
    */
  def delegate[F[_], A](
