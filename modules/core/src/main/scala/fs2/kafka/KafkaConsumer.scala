...
object KafkaConsumer {
  private def startStream[F[_]: Concurrent](stream: Stream[F, Unit]): Resource[F, Fiber[F, Unit]] =
  ...
  private def startPollScheduler[F[_], K, V](
    ...
    startStream(
      Stream.fixedDelay(pollInterval).evalTap { _ =>
