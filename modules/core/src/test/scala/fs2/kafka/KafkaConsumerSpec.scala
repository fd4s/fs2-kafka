...
final class KafkaConsumerSpec extends BaseKafkaSpec2 {
    ...
    it("should consume all records with subscribing for several consumers") {
      withKafka { topic =>
        ...
        val consumed =
          KafkaConsumer
            .stream[IO]
            .using(consumerSettings[IO].withGroupId("test"))
            .evalTap(_.subscribeTo(topic))
            .evalMap(IO.sleep(3.seconds).as(_)) // sleep a bit to trigger potential race condition with _.stream
            .flatMap(_.stream)
            .map(committable => committable.record.key -> committable.record.value)
    ...
    it("should consume records with assign by partitions") {
      withKafka { topic =>
        ...
        val consumed =
          KafkaConsumer
            .stream[IO]
            .using(consumerSettings[IO].withGroupId("test2"))
  ...
  private def commitTest(
    ...
    withKafka { topic =>
      ...
      val actuallyCommitted = withKafkaConsumer(consumerProperties) { consumer =>
        consumer
