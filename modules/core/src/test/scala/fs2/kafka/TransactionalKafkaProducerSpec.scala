  ...
  private def testSingle(topic: String, makeOffset: Option[Long => CommittableOffset[IO]]) = {
    ...
    val produced =
      (for {
        ...
        records <- Stream.chunk(Chunk.seq(toProduce)).zipWithIndex.map { case ((key, value), i) =>
          ...
          ](Left(ProducerRecords.one(record, (key, value))))(offset =>
            Right(
    ...
    withTopic { topic =>
      ...
      val produced =
        (for {
          ...
          records = TransactionalProducerRecords(
            Chunk.seq(recordsToProduce.zip(offsets)).map { case (record, offset) =>
